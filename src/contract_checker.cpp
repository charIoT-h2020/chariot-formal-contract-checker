...
void
processor_set_loader_alloc_shift(struct _PProcessor* aprocessor, uint64_t shift)
{  try {
   Processor* processor = reinterpret_cast<Processor*>(aprocessor);
   processor->setLoaderAllocShift(shift);
   }
   catch (ESPreconditionError& error) {
     std::cerr << "unable to set an instruction shift for the loader!\n";
     error.print(std::cerr);
     std::cerr.flush();
   }
   catch (...) {
     std::cerr << "unable to set an instruction shift for the loader!" << std::endl;
   }
}
...
bool contracts_has_alloc_shift(struct _ContractGraphContent* acontracts)
{  return reinterpret_cast<ContractGraph*>(acontracts)->hasAllocShift(); }

uint64_t contracts_get_alloc_shift(struct _ContractGraphContent* acontracts)
{  return reinterpret_cast<ContractGraph*>(acontracts)->getAllocShift(); }
